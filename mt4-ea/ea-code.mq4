...
void HandleHistoryRequest() {
    ...
    for(int i = 0; i < total; i++) {
        if(OrderSelect(i, SELECT_BY_POS, MODE_HISTORY)) {
            // Format: HISTORY|ticket|symbol|type|openTime|closeTime|openPrice|closePrice|lots|profit|commission|swap
            if(i > 0) response += ";";
...
//+------------------------------------------------------------------+
void HandleGetTradeHistory() {
    string response = "HISTORY";
    bool firstTrade = true;
    
    // Loop through closed orders history
    for(int i = OrdersHistoryTotal() - 1; i >= 0; i--) {
        if(!OrderSelect(i, SELECT_BY_POS, MODE_HISTORY)) continue;
        
        // Skip if not a market order
        if(OrderType() != OP_BUY && OrderType() != OP_SELL) continue;
        
        // Add separator between trades
        if(!firstTrade) response += ";";
        firstTrade = false;
        
        // Format: HISTORY|ticket|symbol|type|openTime|closeTime|openPrice|closePrice|lots|profit|commission|swap
        response += StringFormat("|%d|%s|%d|%s|%s|%f|%f|%f|%f|%f|%f",
            OrderTicket(),
            OrderSymbol(),
            OrderType(),
...
void OnWebSocketMessage(string message) {
    ...
    if(StringFind(message, "COMMAND|") == 0) {
        command = StringSubstr(message, 8);
    }
